...
const ICONO_ESTADO: Record<EstadoServidor, string> = {
  activo: '●', degradado: '◐', inactivo: '○', error: '✕',
}
...
function TarjetaServidor({ nombre, ip, sistema, puerto }: TarjetaServidorProps) {
  return (
    <View style={estilos1.tarjeta}>
      ...
      <Text style={estilos1.datoDireccion}>{ip}:{puerto}</Text>
...
function Paso1() {
  return (
    <View style={estilos1.fondo}>
      ...
      <TarjetaServidor nombre="db-primary"  ip="10.0.2.20" sistema="Debian 12"        puerto={5432} />
      <TarjetaServidor nombre="cache-01"    ip="10.0.2.30" sistema="Alpine 3.19"       puerto={6379} />
    </View>
  )
}
    ...
    <SafeAreaView style={estilos5.safeArea}>
      ...
      <View>
        <Pressable
          ...
          onPress={() => { setAccionEjecutada(null); setModalVisible(true) }}
        >
...
function BotonAccion({ etiqueta, color, onPress }: { etiqueta: string; color: string; onPress: () => void }) {
  return (
    <Pressable
      style={({ pressed }) => [estilos6.botonAccion, { backgroundColor: color }, pressed && { opacity: 0.75 }]}
      onPress={onPress}
    >
...
function Paso6() {
  ...
  function ejecutarAccion() {
    setLog((prev) => [`[${new Date().toLocaleTimeString()}] ${accionPendiente} ejecutado en web-01`, ...prev])
    setModalVisible(false)
  }
      ...
      <ScrollView contentContainerStyle={estilos6.scrollContenido} showsVerticalScrollIndicator={false}>
        ...
        <Card titulo="Acciones">
          <View style={estilos6.gridAcciones}>
            ...
            <BotonAccion etiqueta="Ver logs"   color="#6a1b9a" onPress={() => pedirConfirmacion('Descarga de logs')} />
          </View>
        </Card>
        {log.length > 0 && (
          <Card titulo="Actividad reciente">
            {log.map((e, i) => <Text key={i} style={estilos6.entradaLog}>{e}</Text>)}
          </Card>
        )}
      </ScrollView>
      <ModalConfirm
...
const estilosCard = StyleSheet.create({
  card: { backgroundColor: '#ffffff', borderRadius: 12, overflow: 'hidden', shadowColor: '#000', shadowOpacity: 0.07, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 3 },
...
const estilosModal = StyleSheet.create({
  fondo: { flex: 1, backgroundColor: 'rgba(0,0,0,0.55)', justifyContent: 'center', alignItems: 'center' },
  dialogo: { backgroundColor: '#ffffff', borderRadius: 16, padding: 24, width: '85%', gap: 12, shadowColor: '#000', shadowOpacity: 0.2, shadowRadius: 12, shadowOffset: { width: 0, height: 6 }, elevation: 10 },
...
const estilos1 = StyleSheet.create({
  ...
  tarjeta: { backgroundColor: '#ffffff', borderRadius: 10, padding: 16, gap: 4, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 4, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
...
const estilos3 = StyleSheet.create({
  ...
  fila: { backgroundColor: '#ffffff', borderRadius: 10, paddingHorizontal: 16, paddingVertical: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 3, shadowOffset: { width: 0, height: 1 }, elevation: 1 },
...
const estilos4 = StyleSheet.create({
  ...
  header: { paddingHorizontal: 20, paddingTop: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: '#1c2e40' },
...
const estilos6 = StyleSheet.create({
  ...
  cabecera: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 14, backgroundColor: '#ffffff', borderBottomWidth: 1, borderBottomColor: '#e0e7ef' },
